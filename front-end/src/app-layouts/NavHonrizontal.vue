<template>
	<div class="flex justify-between py-4 px-4 bg-gray-50 shadow-md">
		...
		<div
			...
			:class="[
				'fixed top-0 right-0 z-40 w-96 h-screen p-4 overflow-y-auto transition-transform bg-white shadow-xl',
		...
		>
			...
			<div class="py-4 overflow-y-auto">
				<NotificationComponent v-if="store.currentAccount" />
			</div>
		</div>
...
const createNewAccount = async () => {
	try {
		await axios.post('https://splaika.com/api/v1/accounts', {
...
const deleteMyAccount = async () => {
	try {
		...
		const deleteAccount = await axios.delete(
			'https://splaika.com/api/v1/accounts/deleteAccountById',
		...
		console.log('Hee awijfiowajfwioaj', error);
	}
};
